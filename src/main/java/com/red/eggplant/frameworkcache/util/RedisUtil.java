package com.red.eggplant.frameworkcache.util;
...
public class RedisUtil {
    ...
    private static void initialPool() {
        //Redis服务器IP
        String HOST = PropertiesUtil.getString("redis.native.host");
        //Redis的端口号
        int PORT = PropertiesUtil.getInt("redis.native.port",6379);
        //访问密码
        String AUTH = PropertiesUtil.getString("redis.native.password");

        try {
            JedisPoolConfig config = new JedisPoolConfig();
            //最大连接数，如果赋值为-1，则表示不限制；如果pool已经分配了maxActive个jedis实例，则此时pool的状态为exhausted(耗尽)。
            config.setMaxTotal(PropertiesUtil.getInt("redis.native.maxTotal"));
            //最大空闲数，控制一个pool最多有多少个状态为idle(空闲的)的jedis实例，默认值也是8。
            config.setMaxIdle(PropertiesUtil.getInt("redis.native.maxIdle"));
            //最小空闲数
            config.setMinIdle(PropertiesUtil.getInt("redis.native.minIdle"));
